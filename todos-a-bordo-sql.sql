...
matriz_precios AS (
    SELECT
        ZONA_PROYECTO,
        SUM(CASE WHEN rango_precio = 'Budget (<200€)' THEN 1 ELSE 0 END) AS num_budget,
        SUM(CASE WHEN rango_precio = 'Mid (200-500€)' THEN 1 ELSE 0 END) AS num_mid,
        SUM(CASE WHEN rango_precio = 'Premium (>500€)' THEN 1 ELSE 0 END) AS num_premium,
        COUNT(*) AS total_barcos
    FROM barcos_categorizados
    GROUP BY ZONA_PROYECTO
)
...
SELECT
    ...
    num_premium,
    total_barcos,
    ROUND(num_budget * 100.0 / total_barcos, 1)   AS pct_budget,
    ROUND(num_mid * 100.0 / total_barcos, 1)      AS pct_mid,
    ROUND(num_premium * 100.0 / total_barcos, 1)  AS pct_premium
FROM matriz_precios
ORDER BY ZONA_PROYECTO;
